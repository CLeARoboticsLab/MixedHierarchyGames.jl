...
function compute_z_ordering_for_player(player_id::Int)
    Z_order = []

    # 1. Include the primal control variables for the player.

    # Get all leaders and followers of the player.
    leaders = get_all_leaders(g, player_id)
    followers = get_all_followers(g, player_id)

    # 2. Include the dual variables for the player (always dynamics, sometimes all follower policy constraints, TODO: next stage policy).
    # 3. Include the follower primals and duals, in depth-first order.
    # 4. Include the Nash control variables for player which are neither leaders nor followers.
    # 5. Include the next state variable for the player.
end

# Only include other agent if it is included through objective or constraints.
Zs = Dict(
        1 => [:u1, :λ1, :xtp1], # P1
        3 => [:u3, :λ3, :xtp1], # P3
        2 => [:u2, :λ2, :η23, :u3, :λ3, :xtp1],  # P2
...
# 1a. Gradient of Lagrangian of i wrt to primal control i = 0
# 1b. Gradient of Lagrangian of i wrt to follower control j = 0
# 1c. Policy constraint for follower j's control
# 2a. Gradient of Lagrangian of i wrt to xtp1
# 3. Dynamics constraint
...
M1[Block(4), Block(5)] = I(n); # xtp1

N1[Block(4), Block(1)] = -A;   # xt

# Row 5: ∇L¹ wrt xtp1
M1[Block(5), Block(2)] = I(n);        # λ¹
M1[Block(5), Block(5)] = Qs[1]; # xtp1
...
M3[Block(3), Block(4)] = I(n); # xtp1

N3[Block(3), Block(1)] = -A;  # xt
N3[Block(3), Block(2)] = -B2; # u²

# Row 4: ∇L³ wrt xtp1
M3[Block(4), Block(2)] = I(n);     # λ³
M3[Block(4), Block(4)] = Qs[3]; # xtp1
...
M2[Block(4), Block(7)] = I(n); # xtp1

N2[Block(4), :] = -A;          # xt

# Row 5: ∇L² wrt xtp1
M2[Block(5), Block(2)] = I(n);     # λ²
M2[Block(5), Block(7)] = Qs[2]; # xtp1

# Row 6: ∇L³ wrt xtp1
M2[Block(6), Block(5)] = I(n);     # λ³
M2[Block(6), Block(7)] = Qs[3]; # xtp1

# Row 7: policy constraint
M2[Block(7), Block(1)] = PP3;  # u²
M2[Block(7), Block(4)] = I(mⁱ); # u³
...
P2 = nothing # No control information provided to player 2
...
PP2 = nothing # No control information provided to player 2
...
M12[Block(4), Block(8)] = I(n); # xtp1

N12[Block(4), Block(1)] = -A;   # xt

# Row 5: ∇L¹ wrt xtp1
M12[Block(5), Block(2)] = I(n);     # λ¹
M12[Block(5), Block(8)] = Qs[1]; # xtp1

# Row 6: ∇L² wrt xtp1
M12[Block(6), Block(4)] = I(n);     # λ²
M12[Block(6), Block(8)] = Qs[2]; # xtp1

# Row 7: ∇L³ wrt xtp1
M12[Block(7), Block(7)] = I(n);     # λ³
M12[Block(7), Block(8)] = Qs[3]; # xtp1

# Row 8: leader policy constraint π³
M12[Block(8), Block(3)] = PP3;  # u²
M12[Block(8), Block(6)] = I(mⁱ); # u³
