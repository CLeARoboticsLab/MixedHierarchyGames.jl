#=
    Benchmark: cubic interpolation linesearch vs all other methods

    Compares solve time, iterations, and linesearch statistics across
    all available linesearch methods including cubic interpolation.

    Usage: julia --project=. scripts/benchmark_cubic_linesearch.jl
=#
...
println("=" ^ 70)
println("Cubic Interpolation Linesearch Benchmark")
println("=" ^ 70)
println()
...
methods = [:geometric, :armijo, :armijo_quadratic, :armijo_interp]
