#=
    Benchmark: warm-start α for linesearch

    Compares linesearch performance with warm-start α (min(1.0, 2*α_prev))
    vs baseline (always α_init=1.0). Measures solve time, iterations,
    allocations, and mean backtracks per iteration.

    Usage: julia --project=. scripts/benchmark_warmstart_alpha.jl
=#
...
function benchmark_solve(prob; n_solves=20, linesearch_method=:geometric)
    ...
    # Compute linesearch statistics from step sizes
    # Mean backtracks per iteration: log(α)/log(0.5) gives number of halvings
...
println("=" ^ 70)
println("Warm-start α Benchmark")
println("=" ^ 70)
println()
println("Note: warm-start is always active in the current implementation.")
println("This benchmark reports linesearch statistics to characterize")
println("step size behavior across different problems and methods.")
println()
