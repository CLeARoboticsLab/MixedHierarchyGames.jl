#=
    Benchmark: all linesearch methods including two-phase

    Compares solve time, iterations, and linesearch statistics across
    all available linesearch methods.

    Usage: julia --project=. scripts/benchmark_twophase_linesearch.jl
=#
...
println("=" ^ 70)
println("Two-Phase Linesearch Benchmark (All Methods)")
println("=" ^ 70)
println()
...
methods = [:geometric, :armijo, :armijo_quadratic, :armijo_interp, :twophase]
